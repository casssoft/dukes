...
import qualified OtherSideOfHole as OS

-- IN THE MIDDLE OF REFACTORING SO THAT DIFFERENT PLACES
-- can be in different files
-- but there is a lot of shared code and dependancies
-- We need this logic fun to be independent of available npcs and scenes?
-- Otherwise it will be a circular dep:
-- Scene says hey meet this npc
-- NpcLogic says ok lets go to that npc meet scene in Scene
--
-- So maybe the solution is to do something like this:
-- NpcLogicMeetNpcs datastructFromScene
-- So the npc logic and datastructure is independent but the
